...
var ICON_OPTIONS = [
  ...
  { value: 'ri-robot-2-line', label: 'Robot' },
  { value: 'ri-chat-ai-line', label: 'AI Chat' },
  ...
  { value: 'ri-shopping-cart-line', label: 'Cart' },
...
var bookmarksList = document.getElementById('sidebar-bookmarks-list')
var addBookmarkBtn = document.getElementById('add-sidebar-bookmark')

function getBookmarks () {
  return settings.get('sidebarBookmarks') || DEFAULT_BOOKMARKS
}

function saveBookmarks (bookmarks) {
  settings.set('sidebarBookmarks', bookmarks)
}
...
function createBookmarkItem (bookmark, index) {
  ...
  iconPreview.innerHTML = '<i class="' + (bookmark.icon || 'ri-links-line') + '"></i>'
  ...
  titleInput.addEventListener('input', function () {
    ...
    bookmarks[index].title = this.value
    saveBookmarks(bookmarks)
  })

  urlInput.addEventListener('input', function () {
    var bookmarks = getBookmarks()
    bookmarks[index].url = this.value
    saveBookmarks(bookmarks)
  })

  iconSelect.addEventListener('change', function () {
    var bookmarks = getBookmarks()
    bookmarks[index].icon = this.value
    saveBookmarks(bookmarks)
...
function renderBookmarks () {
  ...
  bookmarks.forEach(function (bookmark, index) {
    var item = createBookmarkItem(bookmark, index)
...
addBookmarkBtn.addEventListener('click', function () {
  var bookmarks = getBookmarks()
  bookmarks.push({ url: '', title: '', icon: 'ri-links-line' })
  saveBookmarks(bookmarks)
  renderBookmarks()
})
